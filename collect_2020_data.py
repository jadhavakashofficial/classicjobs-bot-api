...
# ✅ Add clean_text inline
def clean_text(text):
    return text.replace('\n', ' ').strip()

os.makedirs("bot_training_logs", exist_ok=True)

def collect_2020_data():
    print("📥 Collecting all videos from 2020...")
    ...
    path = "bot_training_logs/2020_combined.txt"
    with open(path, "w", encoding="utf-8") as f:
        f.write("\n".join(combined_lines))

    print(f"✅ Saved combined data for 2020 → {path}")
